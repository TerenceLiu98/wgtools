...
def node(filename:str="wg0", nodename:str="node1"):
    ...
    node = Node()
    node.Name = nodename
    node.Address = "".join(config["Network"]["addr4Pool"][1:-5] + str(random.randint(1, 255)) + "/24")
    node.ListenPort = random.randint(32768, 60999)
    node.PrivateKey, node.PublicKey = wgc_genkey()
    node.AllowedIPs = "".join(node.Address[0][:-3] + "/32")
    node_info = "\n[{}]\n".format(node.Name) + to_toml(node)
    with open("{}.conf".format(filename), "a", newline='') as f:
        f.write(node_info)
    return 0
